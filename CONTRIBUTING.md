...
configure beman.at_most to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_AT_MOST`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_AT_MOST" -C 2
```
...
### `BEMAN_AT_MOST_BUILD_TESTS`

Enable building tests and test infrastructure. Default: `ON`.
Values: `{ ON, OFF }`.

### `BEMAN_AT_MOST_BUILD_EXAMPLES`

Enable building examples. Default: `ON`. Values: `{ ON, OFF }`.

### `BEMAN_AT_MOST_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.at_most` can use
`find_package(beman.at_most)` to locate the library.
